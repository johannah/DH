...
def run_eval(num_train_steps, num_eval_episodes=10):
    ...
    pickle.dump(replay_buffer, open(modelbase+'.pkl', 'wb'))
    



def run_train(num_steps=0, num_episodes=100000):
...
if __name__ == '__main__':
    ...
    exp_name = '%s_%s_%05d'%(args.env, args.task, args.seed)
    ...
    if args.eval:
        ...
        for k in use_states:
            ...
            st = en 
            en += 2
        plotting.plot_loss_dict(policy, modelbase) 
        plotting.plot_replay_reward(replay_buffer, modelbase, start_step=0)
        plotting.plot_states(replay_buffer.get_last_steps(replay_buffer.size), 
                             modelbase, detail_dict=state_names_dict)

        plotting.plot_actions(replay_buffer.get_last_steps(replay_buffer.size), 
                             modelbase)
        if use_frames:
            plotting.plot_frames(modelbase+'_movie.mp4', replay_buffer.get_last_steps(1000), min_action=min_action, max_action=max_action, plot_action_frames=True)
